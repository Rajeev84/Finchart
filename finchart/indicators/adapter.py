...
class IndicatorAdapter:
    ...
    def __init__(
        ...
        self._last_data: Optional[List[OHLCV]] = None
    ...
    def calculate(self, data: List[OHLCV]) -> Optional[pd.Series]:
        ...
        """
        # Store data for later use in on_render
    ...
    def render(
        ...
        # Convert data to DataFrame for on_render
        df = self._ohlcv_to_dataframe(self._last_data) if self._last_data else None
        
        # Let indicator draw custom elements (hlines, bands, etc.)
        self._indicator_class.on_render(proxy, self._subplot, self._params, df)
        
        # AUTO-RENDER: If calculate() produced a Series and on_render() didn't
        # already draw a series with the same name, render it as a line.
        if self._last_series is not None and not self._last_series.empty:
            series_name = self._indicator_class.name.lower().replace(' ', '_')
            existing_tags = [c.tag for c in proxy.get_commands()
                             if c.tag.startswith(f'series_{series_name}')]
            if not existing_tags:
                color = self._params.get('color',
                             self._indicator_class.defaults.get('color', '#2196F3'))
                width = self._params.get('width',
                             self._indicator_class.defaults.get('width', 1.5))
                # Use histogram style for Volume indicator
                style = 'histogram' if self._indicator_class.name == 'Volume' else 'line'
                proxy.create_series(series_name, self._last_series,
